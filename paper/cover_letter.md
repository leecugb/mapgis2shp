...
Several open-source MapGIS readers already exist, including the first author's own earlier `pymapgis` (2022). However, **none has been independently validated against the reference implementation, none infers the coordinate reference system, and none has been peer-reviewed.** The submitted manuscript closes all three gaps:

1. **First peer-reviewed software paper** on a MapGIS 6.x/67 reader (Crossref, arXiv, OpenAlex, and a manual CNKI/Wanfang search return no such paper).

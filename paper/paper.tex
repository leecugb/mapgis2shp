% mapgis2shp paper -- manuscript for Earth Science Informatics (Springer).
% Uses the Springer Nature sn-jnl class (sn-jnl.cls included in the package).
% Preamble mirrors the official sn-article.tex sample (known to compile).
...
% ---- Metadata ----
\title[mapgis2shp: an open-source MapGIS reader]{mapgis2shp: an open-source Python reader for the MapGIS 6.x/67 binary vector formats, validated against the native export}

\author*[1]{\fnm{Shijie} \sur{Li}}\email{1045105061@qq.com}
\author*[1]{\fnm{Haiyang} \sur{He}}\email{hehaiyangcugb@foxmail.com}
\author[1]{\fnm{Xu} \sur{Sun}}
\author[1]{\fnm{Haoyang} \sur{Qin}}
\author[1]{\fnm{Xiaoyu} \sur{Liu}}
\author[1]{\fnm{Yilin} \sur{Feng}}
\author[1]{\fnm{Zengyun} \sur{Zuo}}
\affil[1]{\orgname{Xi'an Mineral Resources Survey, China Geological Survey}, \orgaddress{\city{Xi'an}, \country{China}}}
...
\texttt{pymapgis}~\cite{pymapgis2022}, which first published a byte-level
format description; the present paper adds coordinate-reference inference, a
vectorised parser, polygon-topology reconstruction, and---critically---independent
...
\texttt{BenChao1998/ConvertMapGIS}~\cite{convertmapgis2025} (2025) is an
independent third-party derivative of \texttt{pymapgis}---not authored by the
present authors---that republishes a detailed byte-level specification in its
README and adds a graphical converter.
...
\begin{enumerate}
  ...
  reader. Crossref, arXiv, OpenAlex, and a manual search of CNKI/Wanfang
  return no such paper (Section~\ref{sec:discussion}).
...
coordinate-reference-system bytes (projection@109, ellipsoid@110, scale@143,
central meridian@151 in \texttt{DDDMMSS.sss}), and the 10-entry index area
located at \texttt{data\_start}.
...
MapGIS 6.7 software (Zondy Cyber) under default export settings. The two outputs were then aligned by
...
Section~\ref{sec:related} is fully covered, and no further prior art
affecting the novelty claims was identified.
...
The 36 MapGIS validation layers and the 400~MB extreme-scale file are real
geological-survey data from the China Geological Survey and are not
...
\texttt{cross\_validate.py} for the large-file coverage-equivalence test),
the per-layer results (\texttt{cross\_validation\_report.csv}), the
large-file results (\texttt{cross\_validate\_large\_report.md}), the
regression baseline (\texttt{pymapgis\_baseline.json}), and synthetic minimal
test fixtures (\texttt{tests/}) are available in the \texttt{mapgis2shp}
repository at \url{https://github.com/leecugb/mapgis2shp} and in the archived
release at \url{https://doi.org/10.5281/zenodo.21487339}.

% =============================================================================
\section*{CRediT author statement}
% =============================================================================
S.L. conceived and designed the study, developed the software, carried out
the validation, and wrote the main manuscript text; S.L. also prepared
Figures~1 and~2. H.H. contributed to the validation and reviewed the
manuscript. X.S., H.Q., X.L., Y.F. and Z.Z. contributed to data
investigation and reviewed the manuscript. All authors reviewed and approved
